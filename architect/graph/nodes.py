...
def node_execute(state: PipelineState) -> dict:
    ...
    Temporal runs each agent task as an Activity.
    If the process crashes mid-execution, Temporal resumes from
    the last completed activity on restart.

    In Phase 1 execution: Temporal activities simulate agent calls
    (real agent connector calls come in a later phase).
    """
    import asyncio
    ...
    try:
        # Run the Temporal workflow synchronously from within the node
        result = asyncio.run(
            run_job_workflow(
                job         = state["job"],
